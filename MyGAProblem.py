...
class MyProblem(ea.Problem):  # 继承Problem父类
    def __init__(self, PoolType, cfg):  # PoolType是取值为'Process'或'Thread'的字符串
        ...
        Dim = 4 #变量的个数
        varTypes = [1,1,1,0] #初始化varTypes（决策变量的类型，元素为0表示对应的变量是连续的；1表示是离散的）
        lb = [cfg.PARA.params.num_layers[0],
              cfg.PARA.params.num_neurons[0],
              cfg.PARA.params.batch_size[0],
              cfg.PARA.params.lr[0]]
        ub = [cfg.PARA.params.num_layers[1],
              cfg.PARA.params.num_neurons[1],
              cfg.PARA.params.batch_size[1],
              cfg.PARA.params.lr[1]]
    ...
    def aimFunc(self, pop):  # 目标函数，采用多线程加速计算
        num_layer = pop.Phen[:,[0]]
        num_neuron = pop.Phen[:,[1]]
        batch_size = pop.Phen[:,[2]] * 100
        lr = pop.Phen[:,[3]]
